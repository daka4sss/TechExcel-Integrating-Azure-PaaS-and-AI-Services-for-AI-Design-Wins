...
var config = new ConfigurationBuilder()
    .AddUserSecrets<Program>() // ユーザーシークレットから設定を読み込む
    .AddEnvironmentVariables() // 環境変数から設定を読み込む
    .Build();                  // 設定をビルドして「config」に保存
...
builder.Services.AddSingleton<Kernel>((_) =>
{
    ...
    kernelBuilder.Plugins.AddFromType<DatabaseService>(); // DatabaseServiceをプラグインとして追加
